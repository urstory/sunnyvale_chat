...
public class FacebookOAuth2LoginAuthenticationProcessingFilter extends OAuth2ClientAuthenticationProcessingFilter {

    private ObjectMapper mapper = new ObjectMapper();
    private SocialService socialService;

    public FacebookOAuth2LoginAuthenticationProcessingFilter(String processUrl, SocialService socialService) {
        super(processUrl);
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
        ...
        try {
            final UsernamePasswordAuthenticationToken authenticationToken = socialService.doLogin(ProviderType.FACEBOOK, userConnection);

            super.successfulAuthentication(request, response, chain, authenticationToken);
        }catch(NotFoundSocialInfoException nfsie){
            response.sendRedirect("/errors/notFoundSocialInfo");
