...
public class FacebookOAuth2ConnectAuthenticationProcessingFilter extends OAuth2ClientAuthenticationProcessingFilter {

    private ObjectMapper mapper = new ObjectMapper();
    private SocialService socialService;

    public FacebookOAuth2ConnectAuthenticationProcessingFilter(String processUrl, SocialService socialService) {
        super(processUrl);
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
        ...
        if(authentication == null)
            response.sendRedirect("/errors/notLogin");
        ...
        try {
            final UsernamePasswordAuthenticationToken authenticationToken = socialService.connect(ProviderType.FACEBOOK, userConnection);
